        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                ...
                for _, (x_train, y_train) in enumerate(train_loader):
                    ...
                    optim.zero_grad()
                    confounder = x[:, -1].long()
                    pw_pos = self.propensity_pos[confounder]  # inverse of 
                    pw_neg = self.propensity_neg[confounder]
                    pw = y.squeeze() * pw_pos + (1-y.squeeze()) * pw_neg 
                    pw = 1 / pw
                    loss = loss_func(y_pred, y.squeeze(),weight=pw, reduction='sum')
...
class ipw_myNFM(ipwbasemodel):
    def __init__(self,
                 linear_feature_columns, dnn_feature_columns, use_fm=True,
                 dnn_hidden_units=(256, 128),
                 l2_reg_linear=0.00001, l2_reg_embedding=0.00001, l2_reg_dnn=0, init_std=0.0001, seed=1024,propensity_pos=None,propensity_neg=None,
                 dnn_dropout=0,
                 dnn_activation='relu', dnn_use_bn=False, task='binary', device='cpu', gpus=None, writer=None,emb_dim = None,confounder_name=None):

        super(ipw_myNFM, self).__init__(linear_feature_columns, dnn_feature_columns, l2_reg_linear=l2_reg_linear,
        ...
        self.use_dnn = len(dnn_feature_columns) > 0 and len(
            dnn_hidden_units) > 0
        self.propensity_pos = torch.from_numpy(propensity_pos).float().squeeze().to(self.device)
        self.propensity_neg = torch.from_numpy(propensity_neg).float().squeeze().to(self.device)
        self.confounder_name = confounder_name
        print("propensity need grad?: ", self.propensity_pos.requires_grad)
        if use_fm:
